...
class MonPolicy(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        price_changes_ij = [self.mu_ij[i] / mu_ij_old[i] for i in range(self.n_agents)]
        price_changes = list(filter(lambda x: x != 1, price_changes_ij))
        price_changes_ij_high = [
            price_changes_ij[i]
            if high_mu_ind[i] == 1 and price_changes_ij[i] != 1
            else 0
            for i in range(self.n_agents)
        ]
        price_changes_high = list(filter(lambda x: x != 0, price_changes_ij_high))

        price_changes_ij_low = [
            price_changes_ij[i]
            if high_mu_ind[i] == 0 and price_changes_ij[i] != 1
            else 0
            for i in range(self.n_agents)
        ]

        price_changes_low = list(filter(lambda x: x != 0, price_changes_ij_low))
        ...
        if self.analysis_mode or self.eval_mode:
            ...
            info_global = {
                0: {
                    ...
                    "mean_p_change_low": np.mean(
                        [abs(np.log(elem)) for elem in price_changes_low]
                    ),
                    "mean_p_change_high": np.mean(
                        [abs(np.log(elem)) for elem in price_changes_high]
                    ),
                    "mu_ij": self.mu_ij[0],
