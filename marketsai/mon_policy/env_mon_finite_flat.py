...
class MonPolicyFinite(MultiAgentEnv):
    ...
    def __init__(
        ...
        if self.obs_flex_index and self.regime_change:
            ...
            high = np.array(
                [10 for i in range(self.n_firms)] + [5, float("inf"), self.horizon, 1]
            )
    ...
    def step(self, action_dict):
        ...
        self.move_ij = [
            True
            if self.menu_cost[i]
            <= (action_dict[i][1] + 1) / 2 * self.params["menu_cost"]
            else False
            for i in range(self.n_agents)
        ]
        self.mu_ij_reset = [
            self.markup_min
            + (action_dict[i][0] + 1) / 2 * (self.markup_max - self.markup_min)
            for i in range(self.n_agents)
        ]
