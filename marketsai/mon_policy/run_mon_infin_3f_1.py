...
if RUN_ANALYSIS:
    ...
    if not OBS_IDSHOCK:
        ...
        obs_reaction_medmu = [
            np.array(
                [markup[i], 1.3, 1.3, 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
    ...
    if not OBS_IDSHOCK:
        obs_monpol_lowmu = [
            np.array([1.1, 1.3, 1.3] + [1.16, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
        obs_monpol_medmu = [
    ...
    if not OBS_IDSHOCK:
        obs_reaction_lowmu = [
            np.array(
                [1.2, markup[i], 1.3] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        ...
        obs_reaction_lowmu = [
            np.array(
                [1.2, markup[i], 1.3 , 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
    ...
    if OBS_IDSHOCK:
        ...
        obs_reaction_zshock = [
            np.array(
                [1.3, markup_z[i], 1, z[i]] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_stratdev = [
            np.array(
                [1.3, markup_z[i], 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
