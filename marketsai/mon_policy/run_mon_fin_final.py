...
""" Reaction function to own markup """
...
if not OBS_IDSHOCK:
    ...
    obs_reaction_lowmu = [
        {
            "obs_ind": np.array([markup[i], 1.1, 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.2, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            ),
    ...
    obs_reaction_medmu = [
        {
            "obs_ind": np.array([markup[i], 1.3, 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.2, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            ),
...
if not OBS_IDSHOCK:
    ...
    obs_monpol_lowmu = [
        {
            "obs_ind": np.array([1.1, 1.3, 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.23, mon_policy[i]],
                dtype=np.float32,
            ),
    ...
    obs_monpol_medmu = [
        {
            "obs_ind": np.array([1.3, 1.3, 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.23, mon_policy[i]],
                dtype=np.float32,
            ),
    ...
    obs_monpol_highmu = [
        {
            "obs_ind": np.array([1.5, 1.3, 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.23, mon_policy[i]],
                dtype=np.float32,
            ),
...
""" Reaction function to Competition Markup """
markup = [1 + (i / 19) for i in range(20)]
if not OBS_IDSHOCK:
    obs_reaction_lowmu = [
        {
    ...
    obs_reaction_medmu = [
        {
            "obs_ind": np.array([1.3, markup[i], 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.2, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            ),
...
if OBS_IDSHOCK:
    ...
    obs_reaction_zshock = [
        {
            "obs_ind": np.array([1.4, markup_z[i], 1, z[i]], dtype=np.float32),
            "obs_agg": np.array(
                [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            ),
            "time": 24,
            "flex_index": 0,
    ...
    obs_reaction_stratdev = [
        {
            "obs_ind": np.array([1.4, markup_z[i], 1, 1], dtype=np.float32),
            "obs_agg": np.array(
                [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            ),
