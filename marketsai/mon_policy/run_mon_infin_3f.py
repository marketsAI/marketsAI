...
if RUN_ANALYSIS:
    ...
    if not OBS_IDSHOCK:
        ...
        obs_reaction_medmu = [
            np.array(
                [markup[i], 1.3, 1.3] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_highmu = [
            np.array(
                [markup[i], 1.5, 1.5] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        ...
        obs_reaction_lowmu = [
            np.array(
                [markup[i], 1.1, 1.1, 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_medmu = [
            np.array(
                [markup[i], 1.3, 1.3, 1, 1, 2] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_highmu = [
            np.array(
                [markup[i], 1.5, 1.5, 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
    ...
    if not OBS_IDSHOCK:
        obs_monpol_lowmu = [
            np.array([1.1, 1.3, 1.3] + [1.23, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
        obs_monpol_medmu = [
            np.array([1.3, 1.3, 1.3] + [1.23, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
        obs_monpol_highmu = [
            np.array([1.5, 1.3, 1.3] + [1.23, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
    else:
        obs_monpol_lowmu = [
            np.array([1.1, 1.3, 1.3, 1, 1, 1] + [1.23, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
        obs_monpol_medmu = [
            np.array([1.3, 1.3, 1.3, 1, 1, 1] + [1.23, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
        obs_monpol_highmu = [
            np.array([1.5, 1.3, 1.3, 1, 1, 1] + [1.23, mon_policy[i]], dtype=np.float32)
            for i in range(20)
        ]
    ...
    if not OBS_IDSHOCK:
        obs_reaction_lowmu = [
            np.array(
                [1.1, markup[i], 1.3] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_medmu = [
            np.array(
                [1.3, markup[i], 1.3] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_highmu = [
            np.array(
                [1.5, markup[i], 1.3] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        ...
        obs_reaction_lowmu = [
            np.array(
                [1.1, markup[i], 1.3 , 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_medmu = [
            np.array(
                [1.3, markup[i], 1.3, 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
        obs_reaction_highmu = [
            np.array(
                [1.5, markup[i], 1.3, 1, 1, 1] + [1.23, math.e ** env.params["log_g_bar"]],
                dtype=np.float32,
            )
            for i in range(20)
        ]
