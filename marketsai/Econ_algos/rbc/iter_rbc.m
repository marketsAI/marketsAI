function [IterRslt,IterFlag] = iter_rbc(GNDSGE_OPTIONS)
%% Add path
if ispc
...
GNDSGE_DATA = zeros(13,GNDSGE_NPROB);
...
if ~GNDSGE_USE_OLD_VEC
    GNDSGE_SPLINE_VEC = convert_to_interp_eval_array({GNDSGE_PP_c_interp});
end
...
GNDSGE_DATA(:) = [repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);shock_trans(:);z(:); ],1,GNDSGE_NPROB);GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K(:)';   ];
...
[GNDSGE_SOL,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL,GNDSGE_OPT_INFO] = mex_rbc(GNDSGE_SOL,GNDSGE_LB,GNDSGE_UB,GNDSGE_DATA,GNDSGE_SKIP,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL);
GNDSGE_USE_BROYDEN_NOW = 0;
...
IterRslt.output_var_index = output_var_index;
        ...
        IterRslt.params = v2struct(beta,sigma,alpha,delta,GNDSGE_EMPTY);
        IterRslt.var_shock = v2struct(z);
        IterRslt.var_state = v2struct(K);
        IterRslt.var_policy = v2struct(c,K_next);
        IterRslt.var_interp = v2struct(c_interp);
        IterRslt.var_aux = v2struct(w,GNDSGE_EMPTY);
        IterRslt.pp = v2struct(GNDSGE_PP_c_interp,GNDSGE_SPLINE_VEC,GNDSGE_EMPTY);
        ...
        if ~NoSave
            if IterSaveAll
                save(['IterRslt_rbc_' num2str(GNDSGE_Iter) '.mat']);
            else
                save(['IterRslt_rbc_' num2str(GNDSGE_Iter) '.mat'],'IterRslt');
            end
        end
    end
end
