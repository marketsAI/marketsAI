function SimuRslt = simulate_capital_planner_3hh(IterRslt,GNDSGE_OPTIONS)
%% Add path
if ispc
...
if shock_num>1
    GNDSGE_PP=struct('form','MKL','breaks',{{1:shock_num,K_1,K_2,K_3}},...
    ...
    GNDSGE_PP=struct('form','MKL','breaks',{{K_1,K_2,K_3}},...
...
K_3=SimuRslt.K_3(:,GNDSGE_t);
    ...
    GNDSGE_data0 = repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);phi(:);shock_trans(:);z_agg(:);z_ind_1(:);z_ind_2(:);z_ind_3(:); ],1,GNDSGE_NPROB);
...
%     %{
    if GNDSGE_t>0
        if shock_num>1
            GNDSGE_SOL = myinterp_mex(int32(NumThreads),GNDSGE_PP.breaks,GNDSGE_PP.coefs,...
                int32(GNDSGE_PP.pieces),int32(GNDSGE_PP.order),int32(GNDSGE_PP.dim),'not-a-knot',[SimuRslt.shock(:,GNDSGE_t)';SimuRslt.K_1(:,GNDSGE_t)';SimuRslt.K_2(:,GNDSGE_t)';SimuRslt.K_3(:,GNDSGE_t)'],[],[],[]);
        else
            GNDSGE_SOL = myinterp_mex(int32(NumThreads),GNDSGE_PP.breaks,GNDSGE_PP.coefs,...
                int32(GNDSGE_PP.pieces),int32(GNDSGE_PP.order),int32(GNDSGE_PP.dim),'not-a-knot',[SimuRslt.K_1(:,GNDSGE_t)';SimuRslt.K_2(:,GNDSGE_t)';SimuRslt.K_3(:,GNDSGE_t)'],[],[],[]);
        end
    end
    %}
    ...
    % Reconstruct data
    GNDSGE_DATA = [GNDSGE_data0;shock(:)';K_1(:)';K_2(:)';K_3(:)';   ];
...
SimuRslt.K_3(:,GNDSGE_t+1) = K_3_next(:);
    ...
    if mod(GNDSGE_t,SimuSaveFreq)==0
        save(['SimuRslt_capital_planner_3hh_' num2str(GNDSGE_t) '.mat'], 'SimuRslt');
    end
end
end