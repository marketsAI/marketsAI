function [IterRslt,IterFlag] = iter_capital_planner_2hh(GNDSGE_OPTIONS)
%% Add path
if ispc
...
assert(length(z_agg)==8);
assert(length(z_ind_1)==8);
assert(length(z_ind_2)==8);
assert(size(shock_trans,1)==8);
assert(size(shock_trans,2)==8);
...
[GNDSGE_TENSOR_shockIdx,GNDSGE_TENSOR_K_1,GNDSGE_TENSOR_K_2]=ndgrid(1:shock_num,K_1,K_2);
GNDSGE_TENSOR_z_agg=ndgrid(z_agg,K_1,K_2);
GNDSGE_TENSOR_z_ind_1=ndgrid(z_ind_1,K_1,K_2);
GNDSGE_TENSOR_z_ind_2=ndgrid(z_ind_2,K_1,K_2);
...
GNDSGE_UB(2:2,:)=1;
GNDSGE_LB(3:3,:)=0;
GNDSGE_UB(3:3,:)=((2/phi)*GNDSGE_TENSOR_z_agg(:)'.*GNDSGE_TENSOR_z_ind_1(:)'.*GNDSGE_TENSOR_K_1(:)'.^alpha).^(1/2)+(1-delta)*GNDSGE_TENSOR_K_1(:)';
GNDSGE_LB(4:4,:)=0;
GNDSGE_UB(4:4,:)=((2/phi)*GNDSGE_TENSOR_z_agg(:)'.*GNDSGE_TENSOR_z_ind_2(:)'.*GNDSGE_TENSOR_K_2(:)'.^alpha).^(1/2)+(1-delta)*GNDSGE_TENSOR_K_2(:)';
...
GNDSGE_DATA = zeros(97,GNDSGE_NPROB);
...
GNDSGE_PP_s_1_interp=struct('form','pp','breaks',{{K_1,K_2}},'Values',reshape(s_1_interp,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',GNDSGE_INTERP_ORDER,'Method',[],'ExtrapolationOrder',GNDSGE_EXTRAP_ORDER,'thread',NumThreads,'orient','curvefit');
GNDSGE_PP_s_1_interp=myinterp(myinterp(GNDSGE_PP_s_1_interp));

GNDSGE_PP_s_2_interp=struct('form','pp','breaks',{{K_1,K_2}},'Values',reshape(s_2_interp,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',GNDSGE_INTERP_ORDER,'Method',[],'ExtrapolationOrder',GNDSGE_EXTRAP_ORDER,'thread',NumThreads,'orient','curvefit');
...
if ~GNDSGE_USE_OLD_VEC
    GNDSGE_SPLINE_VEC = convert_to_interp_eval_array({GNDSGE_PP_s_1_interp,GNDSGE_PP_s_2_interp});
end
...
if nargin>=1 && isfield(GNDSGE_OPTIONS,'WarmUp')
    if isfield(GNDSGE_OPTIONS.WarmUp,'var_interp')
        ...
        GNDSGE_TEMP = v2struct(K_1,K_2,GNDSGE_SIZE_STATE);
...
if ~GNDSGE_USE_OLD_VEC
    ...
    if REUSE_WARMUP_SOL==1 && isfield(GNDSGE_OPTIONS.WarmUp,'GNDSGE_PROB') && size(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_SOL,1)==size(GNDSGE_SOL,1)
        ...
        v2struct(GNDSGE_OPTIONS.WarmUp.var_state);
        GNDSGE_SOL_interp=struct('form','MKL','breaks',{{[1:shock_num],K_1,K_2}},'Values',reshape(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_SOL,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',[2,2*ones(1,length(GNDSGE_SIZE_STATE))],'Method',[],'ExtrapolationOrder',[2,2*ones(1,length(GNDSGE_SIZE_STATE))],'thread',NumThreads,'orient','curvefit');
        GNDSGE_SOL_interp=myinterp(GNDSGE_SOL_interp);
        GNDSGE_LB_interp=struct('form','MKL','breaks',{{[1:shock_num],K_1,K_2}},'Values',reshape(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_LB,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',[2,2*ones(1,length(GNDSGE_SIZE_STATE))],'Method',[],'ExtrapolationOrder',[2,2*ones(1,length(GNDSGE_SIZE_STATE))],'thread',NumThreads,'orient','curvefit');
        GNDSGE_LB_interp=myinterp(GNDSGE_LB_interp);
        GNDSGE_UB_interp=struct('form','MKL','breaks',{{[1:shock_num],K_1,K_2}},'Values',reshape(GNDSGE_OPTIONS.WarmUp.GNDSGE_PROB.GNDSGE_UB,[],GNDSGE_SIZE_STATE{:}),'coefs',[],'order',[2,2*ones(1,length(GNDSGE_SIZE_STATE))],'Method',[],'ExtrapolationOrder',[2,2*ones(1,length(GNDSGE_SIZE_STATE))],'thread',NumThreads,'orient','curvefit');
        GNDSGE_UB_interp=myinterp(GNDSGE_UB_interp);
        
        v2struct(GNDSGE_TEMP);
        GNDSGE_SOL = reshape(myinterp(GNDSGE_SOL_interp,[GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K_1(:)';GNDSGE_TENSOR_K_2(:)'; ]),size(GNDSGE_SOL));
        GNDSGE_LB_NEW = reshape(myinterp(GNDSGE_LB_interp,[GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K_1(:)';GNDSGE_TENSOR_K_2(:)'; ]),size(GNDSGE_LB));
        GNDSGE_UB_NEW = reshape(myinterp(GNDSGE_UB_interp,[GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K_1(:)';GNDSGE_TENSOR_K_2(:)'; ]),size(GNDSGE_UB));
...
GNDSGE_DATA(:) = [repmat([shock_num;beta(:);sigma(:);alpha(:);delta(:);phi(:);shock_trans(:);z_agg(:);z_ind_1(:);z_ind_2(:); ],1,GNDSGE_NPROB);GNDSGE_TENSOR_shockIdx(:)';GNDSGE_TENSOR_K_1(:)';GNDSGE_TENSOR_K_2(:)';   ];
...
[GNDSGE_SOL,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL,GNDSGE_OPT_INFO] = mex_capital_planner_2hh(GNDSGE_SOL,GNDSGE_LB,GNDSGE_UB,GNDSGE_DATA,GNDSGE_SKIP,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL);
GNDSGE_USE_BROYDEN_NOW = 0;
...
s_1=GNDSGE_SOL(1:1,:);
s_2=GNDSGE_SOL(2:2,:);
K_1_next=GNDSGE_SOL(3:3,:);
K_2_next=GNDSGE_SOL(4:4,:);
...
outputVarStack = cat(1,reshape(s_1,1,[]),reshape(s_2,1,[]));
...
if shock_num>1
    ...
    output_interp=struct('form','MKL','breaks',{{1:shock_num,K_1,K_2}},...
        'Values',outputVarStack,...
        'coefs',[],'order',[2 OutputInterpOrder*ones(1,length({K_1,K_2}))],'Method',[],...
        'ExtrapolationOrder',[],'thread',NumThreads, ...
        'orient','curvefit');
else
    outputVarStack = reshape(outputVarStack, [],GNDSGE_SIZE_STATE{:});
    output_interp=struct('form','MKL','breaks',{{K_1,K_2}},...
        'Values',outputVarStack,...
        'coefs',[],'order',[OutputInterpOrder*ones(1,length({K_1,K_2}))],'Method',[],...
        'ExtrapolationOrder',[],'thread',NumThreads, ...
        'orient','curvefit');
end
...
IterRslt.output_var_index = output_var_index;
        ...
        IterRslt.var_shock = v2struct(z_agg,z_ind_1,z_ind_2);
        IterRslt.var_state = v2struct(K_1,K_2);
        IterRslt.var_policy = v2struct(s_1,s_2,K_1_next,K_2_next);
        IterRslt.var_interp = v2struct(s_1_interp,s_2_interp);
        IterRslt.var_aux = v2struct(GNDSGE_EMPTY);
        IterRslt.pp = v2struct(GNDSGE_PP_s_1_interp,GNDSGE_PP_s_2_interp,GNDSGE_SPLINE_VEC,GNDSGE_EMPTY);
        ...
        if ~NoSave
            if IterSaveAll
                save(['IterRslt_capital_planner_2hh_' num2str(GNDSGE_Iter) '.mat']);
            else
                save(['IterRslt_capital_planner_2hh_' num2str(GNDSGE_Iter) '.mat'],'IterRslt');
            end
        end
    end
end
