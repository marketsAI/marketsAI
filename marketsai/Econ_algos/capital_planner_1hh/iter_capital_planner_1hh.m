function [IterRslt,IterFlag] = iter_capital_planner_1hh(GNDSGE_OPTIONS)
%% Add path
if ispc
...
[GNDSGE_SOL,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL,GNDSGE_OPT_INFO] = mex_capital_planner_1hh(GNDSGE_SOL,GNDSGE_LB,GNDSGE_UB,GNDSGE_DATA,GNDSGE_SKIP,GNDSGE_F,GNDSGE_AUX,GNDSGE_EQVAL);
GNDSGE_USE_BROYDEN_NOW = 0;
...
IterRslt.output_var_index = output_var_index;
        ...
        if ~NoSave
            if IterSaveAll
                save(['IterRslt_capital_planner_1hh_' num2str(GNDSGE_Iter) '.mat']);
            else
                save(['IterRslt_capital_planner_1hh_' num2str(GNDSGE_Iter) '.mat'],'IterRslt');
            end
        end
    end
end
