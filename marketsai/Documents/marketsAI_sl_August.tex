...
\item I introduce Deep Reinforcement Learning (Deep RL) as a "learning from experience" technology, in the spirit of least square learning (e.g., Macet and Sargent (1989)) but with less information about the economy. \medskip

\item  In a simple heterogeneous agents model, I show that Deep RL agents can learn the rational expectations solution and I suggest design choices that aid such learning. \medskip

\item Then, by manipulating our baseline framework I highlight potential applications:   \medskip
\begin{itemize}
	\item high dimensional and incomplete information problems. \ms
	\item equilibrium selection in models with multiple equilibrium. \ms
	\item Strategic problems in dynamic settings. \ms
\end{itemize},
...
\begin{itemize}
	...
	\item Deep RL has three main characteristics: \medskip
	
	\begin{enumerate}
		\item \textbf{Model-free}: The algorithms do not use any mathematical knowledge of  the system they are controlling, other than other than the allowed actions and states. \medskip
		
		\item \textbf{Simulation-Based}: They feed actions to the system and observe the reward they get that period and how the state evolves. With that information, they update their estimations of value function and policy function.  \medskip
		
		\item \textbf{Deep}: they use Deep Neural Nets as function approximators for the value function and/or the policy function.   \medskip
		
	\end{enumerate} 
	


	\item It has shown state of the art performance in many tasks such as: \medskip
	
	\begin{itemize}
		\item Games, including complex multi-agent games. (e.g., Go and Dota 2)\medskip
		\item Robotics and autonomous driving. (e.g., Tesla autopilot and Covariant  robotic arms). \medskip
		\item Operations research and logistics.  (e.g., Deepmind on Google Data Center Cooling). \medskip
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
	...
	\item Reinforcement Learning algorithms in economics: \medskip
	\begin{itemize}
		\item Calvano et al (2020) and Asker, Fershtman and Pakes (2021) uses first generation RL algorithms to study algorithmic collusion in repeated Bertrand games. \medskip
		
		\item Marimon, Sargent and McGrattan (1989) uses a "pre-RL" AI algorithm to study equilibrium selection in a Kiyotaki and Wright (1989) monetary model. \medskip
	\end{itemize}
	
	
	\item Use of neural nets in computation macroeconomics \medskip
	\begin{itemize}
		\item Maliar, Maliar and Winant (2020) solve Krusell and Smith (1998) without using bounded rationality.   \medskip
		
		\item Kahou, Fernandez-Villaverde, Perla and Sood (2021) exploits symmetry in policy functions suing neural nets. \medskip 
		
		\item Azinovic, Gaegauf and Scheidegger (2020) also uses a simulation based approach with neural net approximators to solve the equilibrium differential equations. \medskip	
	\end{itemize}

	\item This paper is the first to use Deep RL algorithms on dynamic stochastic problems. 
...
\begin{itemize}
	\item Rewards engineering is usually the main bottleneck (e.g., in chess). In economics, rewards are well specified in every period. \medskip
...
\item  I present a version of the stochastic growth model with heterogeneous agents in which we focus on the purchase market for capital goods.  \medskip

\item $N^h$ households produce the final good and choose how much consume vs invest. The final good is produced using bundle of $N^c$ capital goods.    \medskip

\item  The cost of producing new capital goods is quadratic in total investment on that good.  \medskip

\item  We will consider both a  market's formulation and a planner's formulation of the problem. \medskip

\item As in Krusell and Smith (1998) model, we have heterogenous agents that accumulate capital and face idiosyncratic risk and aggregate risk. \medskip
\begin{itemize}
\item But, different source of idiosyncratic risk and many separate production functions. \medskip

\item Also, here we model a finite set of households, instead of a continuum. \medskip

\item  Cao (2020) solves the Krusell and Smith (1998) framework with finite agents using the GDSGE toolbox (Cao, Luo, and Nie (2020)). \medskip
\end{itemize}
\end{itemize}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\item Can be implemented as a multi-agent cooperative game with "centralized learning, decentralized execution".
		\end{enumerate}
...
\begin{frame}
\frametitle{Market formulation, $N^h$ HHs, 1 capital: household problem}

\begin{itemize}
	\item  First, we assume that there is a market for investment goods and we introduce capital good firms that pay the adjustment cost and sell the investment good at price $p^k_{t}$. Thus, from the point of view of the household, investment is $I^h_{i,t} = s_{i,t} Y_{i,t}/p^k_{t}$.   \medskip
	
	\item There are $N^h$ households that operate a production fraction and decide how much capital goods to purchase. The recursive formulation of the problem for household $i$ is:
...
\begin{itemize}
	\item It pays a cuadratic adjustment cost $C(I_t)=\frac{\phi}{2} I_{t}^2$. \medskip
	\item It maximizes profits	$$\pi_{t}=p^k_{t} I_{t}-\frac{\phi}{2} I_{t}^2 $$. \medskip
	
	\item Price is then determined by 	 $$p^k_{t} = \phi I_{t}$$ \medskip
...
\begin{itemize}
	\item Then, given decreasing returns to scale, agents need  information on the entire distribution of capitals stock to predict prices. \medskip
\end{itemize}
%\item Later, in order to study strategic behavior in environments where both buyers and sellers are Deep RL agents,  we will allow a deep RL agent to fix the mark-up over price. 
...
\begin{itemize}
\item Now we assume there are $N^c$ capital goods indexed by $j$.   \medskip

\item The recursive formulation of the problem for household $i$ is:

\begin{align*}
V_i \left(\{K_{i,j,t}\}_{i,j} \{ Z^{ind}_{i,t} \}_i, Z_t^{agg}\right) = &\max_{\{s_{i,j,t},K_{i,j,t+1}\}_{j}}  U(C_{i,t}) + \beta \E_t V_i(\{K_{i,j,t+1}\}_{i,j} \{Z^{ind}_{i,t+1} \}_i, Z_{t+1}^{agg}) \qquad \text{s.t.}\\
&\qquad
C_{i,t} = (1-\sum_{j=1}^{N^c} s_{i,j,t})Y_{i,t} \\
&\qquad
Y_{i,t}=Z^{agg}_{t} Z^{id}_{i,t}  \Pi_{j=1}^{N^c} K_{i,j,t}^{\alpha/N^c}\\
&\qquad
K_{i,j,t+1} \leq (1-\delta) K_{i,j,t} + \frac{s_{i,j,t} Y_{i,t}}{p^k_{j,t}}  \quad \text{for } j \in [1,...,N^c] 
\end{align*}

\item The supply is give by the equation $p^k_{j,t} = \phi I^c_{j,t}$
...
%%% SLIDE 2 %%%
%\begin{frame}
%\frametitle{ Planner formulation, 1 Household, 1 capital good}
%\begin{itemize}
...
\begin{frame}
\frametitle{Way Neural Nets?}

\begin{itemize}
	\item They are an universal approximator. \medskip
	
	\item It is easy to impose constraint by manipulating the input or output layer.  \medskip 
	
	\item They are efficient for high dimensional problems (Grohs, Jentzen and Salimova (2019)). \medskip
	
	\item Since they are not aligned to a basis, they satisfy a symmetry property called "isotropy". \medskip
	
	\item  Neural networks are "easy" to compute. There's good software for them and are particularly suited for GPU-acceleration and other computational tools. \medskip
	
	\item There are proofs that in many scenarios for neural networks the local minima are the global minima (Watt, Lacotte and Pilanchi 2020)
\end{itemize}
...
\begin{itemize}
	\item We can compare the discounted utility on a simulated time window of a 1000 periods. \medskip
	\item W calculate the theoretical maximum by solving the model with global methods on a very fine grid using the GDSGE package.
	\begin{figure}
		\centering\includegraphics[scale=0.3]{\dir/econ_vs_RL_learning.png}
	\end{figure}
...
\begin{itemize}
	\item We simulate a series of shocks for 100 periods and compare the optimal policy calculated through policy iteration vs the policy learned trough Deep RL. 
\begin{figure}
	\centering\includegraphics[scale=0.5]{\dir/capital_planner_IRecon_Aug3_1hh.png}
\end{figure}

	\item The consumption paths are very close, but  we still observe differences in time series.
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Planner,no centralized learning.}

\begin{itemize}
	
	\item We now compare scalability in the case when we solve map from the global state to $N^h$ policies (no centralized learning).
	
	\begin{figure}
		\centering\includegraphics[scale=0.28]{\dir/Learning_graph_planner_sa.png}
	\end{figure}
...
%	\begin{figure}
%	\centering\includegraphics[scale=0.4]{\dir/econ_v_srl_1hh_July29.png}
%\end{figure}
%\end{frame}
...
\begin{frame}
\frametitle{Comments on Experiment 1}
